 <?php include ('header.php'); ?>
         <div class="col s12">
          <a href="index.php" class="breadcrumb">Home</a>
          <a href="portfolio.php" class="breadcrumb">Njūsai</a>
          <a href="#!" class="breadcrumb">Njūsai apie ūsus</a>
       </div>
    </nav>
    </div>

   <div class="otherHeaders storiusai">
  <h1><img <?php displayRandomPhotoArea();?>></h1>
    </div>

</header>

 <div class="col s12 m7">
    <h2 class="header kaciuStraipsnis">"Kodėl vyrams auga barzda ir ūsai, o moterims - ne?"</h2>
    <div class="card horizontal">
      <div class="card-image">
        <img  src="images/vyras.jpg">
      </div>
      <div class="card-stacked">
        <div class="card-content">
          <p style="font-weight: bold;">Turite daktaro Fu Manču stiliaus ūsus? Vešlias žandenas? Mažą barzdelę po apatine lūpa? Jei taip, tikriausiai esate vyras, o plaukų augimą skatina jūsų organizme esantys hormonai.</p>

          <p style="margin-top: 24px">Vaikystėje berniukų ir mergaičių kūnus dengia ploni, švelnūs, beveik nematomi plaukeliai, kurie, atėjus paauglystei, pradeda tamsėti ir storėti. Šiuos pokyčius sukelia hormonai androgenai, kurių yra abiejų lyčių organizmuose. Jautriausiai į testosteroną ir kitus androgenus reaguoja pažastų ir gaktos sritys, todėl paauglystėje šiose vietose mergaitėms ir berniukams ima želti tamsūs plaukai. Tačiau vyrų organizmuose yra daugiau androgenų, todėl jų plaukuotumas būna ryškesnis nei moterų. Štai kodėl paaugliams ima kaltis barzda ir ūsai. Plaukai dažniausiai pradeda augti ant viršutinės lūpos, o po kurio laiko - ant skruostų ir smakro. Ir nors plaukų augimą stimuliuoja testosteronas ir kiti vyriški hormonai, jų storį ir tamsumą lemia genetika. Tad jei esate vyras, kuriam nepavyksta užsiauginti “ožio barzdelės”, kaltinkite savo genus, o ne hormonus.</p>
        </div>
      </div>
    </div>
  </div>


    <div class="row">
    <div class="col s12 m6 l6">
      <div class="card">
        <div class="card-image">
          <img src="images/kaciukas.jpg">
          <span class="card-title">"Kas nutiktų, jei katei ar šuniui pašalintumėte ūsus?"</span>
          <a href="story1.php" class="btn-floating halfway-fab waves-effect waves-light red"><i class="material-icons">add</i></a>
        </div>
        <div class="card-content">
          <p>Dažnas yra atkreipęs dėmesį į katės, žiurkės ar šuns ūsus. Dauguma net žino, kad katėms juos nukirpti nėra labai gerai (vienas kitas tokį eksperimentą greičiausiai slapta bandė atlikti vaikystėje). Dėl šunų ūsų jaudinamasi mažiau. Tačiau išties – kas tai per dalykas? Kam jie skirti ir kodėl tiek daug gyvūnų juos turi?</p>
        </div>
      </div>
    </div>
    <div class="col s12 m6 l6">
      <div class="card">
        <div class="card-image">
          <img src="images/barzda.jpg">
          <span class="card-title">"Barzda – tai ne stilius, o įsipareigojimas"</span>
          <a href="story3.php" class="btn-floating halfway-fab waves-effect waves-light red"><i class="material-icons">add</i></a>
        </div>
        <div class="card-content">
          <p>Šiandien pamatyti vyrą su barzda ne toks ir retas reiškinys, nes auginti barzdą pastaruoju metu tapo madinga. Tačiau barzdočius vienijančio „Barzdų klubo“ nariai įsitikinę, kad tai ne tik stilius, bet ir tam tikros atsakomybės prisiėmimas. Stiprią šimtus vyrų subūrusio klubo bendruomenę vienija ne tik barzdos, bet ir bendra socialinė veikla.</p>
        </div>
      </div>
    </div>
 